	...
	if (AbilitySystem)
	{
		...
		if (HasAuthority() && AbilityList.Num() > 0)
		{
			for (auto Ability : AbilityList) {
				if (Ability)
				{
					// UE_LOG(LogTemp, Log, TEXT("BeginPlay GiveAbility %d"), inputID);
					AbilityHandles.Emplace(AbilitySystem->GiveAbility(	FGameplayAbilitySpec(Ability.GetDefaultObject(), 1, inputID++)));
				}
			}
		}
